...
import {EmptyDatatableFilters, OrderGlobalFilters} from "../constants/EmptyDatatableFilters.ts";
import {EmptyFields} from "../constants/EmptyFields.ts";
import {EmptyDatatableOptions} from "../constants/EmptyDatatableOptions.ts";

let filters = ref<DataTableFilterMeta>(EmptyDatatableFilters);
let globalFilters = ref<Array<string>>(OrderGlobalFilters);
let datatableOptions = ref<DatatableOptions>(EmptyDatatableOptions);
let columns = ref<Array<ColumnType>>(EmptyFields);
let customers = ref();
let countries = ref([
    {name: 'USA', code: 'US'},
...
const clearFilter = () => {
    filters.value = EmptyDatatableFilters;
};
...
onMounted(() => {
    loading.value = true;
    EmptyService.getCustomersMedium().then((data) => {
